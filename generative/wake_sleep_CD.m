...
for iter = 1 : param.epochs
    ...
    for b = 1 : batch_num
        ...
        %noise=rand(size(batch_data));
        %noise2add = zeros(size(batch_data));
        %noise2add(noise<0.001)=1;
        %batch_data = batch_data + noise2add;
		...
		for l = 2 : num_layer
            if l == 2
                ...
                presigmoid_sigm = sigmoid(presigmoid);
	            wake_activations{l} = presigmoid_sigm;
        ...
        for l = num_layer - 1 : -1 : 2
			if l == 2
                ...
                border_size = 4;
                presigmoid_sigm(:,1:border_size,:,:)=0;
                presigmoid_sigm(:,:,1:border_size,:)=0;
                presigmoid_sigm(:,:,:,1:border_size)=0;
                presigmoid_sigm(:,end-(border_size-1):end,:,:)=0;
                presigmoid_sigm(:,:,end-(border_size-1):end,:)=0;
                presigmoid_sigm(:,:,:,end-(border_size-1):end)=0;
                
                
				sleep_activations{l-1} = presigmoid_sigm;%sigmoid(presigmoid);
			elseif strcmp(model.layers{l}.type, 'convolution')
        ...
        % apply delta for weights and biases (for top layer)

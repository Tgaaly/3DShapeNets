...
%             border_size = 1;
%             visible_prob(:,1:border_size,:,:,:)=0;
%             visible_prob(:,:,1:border_size,:,:)=0;
%             visible_prob(:,:,:,1:border_size,:)=0;
%             visible_prob(:,end-(border_size-1):end,:,:,:)=0;
%             visible_prob(:,:,end-(border_size-1):end,:,:)=0;
%             visible_prob(:,:,:,end-(border_size-1):end,:)=0;
            
            %%
            %w1 = squeeze(hidden_prob(1,:,:,:,16))%W(1,:,:,:);
%             figure;
%             w1 = squeeze(visible_prob(10,:,:,:,1));%W(1,:,:,:);
...
%             alphamap(0.15 .* alphamap);
    ...
    if mod(iter,5) == 0
        ...
        %w1 = squeeze(hidden_prob(1,:,:,:,16))%W(1,:,:,:);
        figure;
        w1 = squeeze(visible_prob(32,:,:,:,1));%W(1,:,:,:);
        h = vol3d('cdata',w1,'texture','3D');
        colormap(bone(256));
        view(3); % Update view since 'texture' = '2D'
        vol3d(h);  

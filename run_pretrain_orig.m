function model = run_pretrain_orig()
...
param = [];
param.debug = 0;
param.volume_size = 42;%13;%21;%11;%21;%24;
param.pad_size = 3;%4;%5;%3;
data_size = param.volume_size + 2 * param.pad_size;

%% load data
param.data_path = '../data/data_orig';% ~/Dropbox/3dprior/data_CDBM_semantic_v3_semantic_abstract_da';%data_CDBM_semantic_v3_semantic';%data_CDBM_synthetic';%data_CDBM_semantic';
...
param.network = {
    struct('type', 'input');
    struct('type', 'convolution', 'outputMaps', 48, 'kernelSize', 8, 'actFun', 'sigmoid', 'stride', 2);
...
save('pretrained_model_orig','model');

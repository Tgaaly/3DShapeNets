...
% reset(gpuDevice(1))
rng default
run_pretrain_semantic5();


load('pretrained_model_semantic5.mat'); %_synthetic_100acc_largenet');

rec_train(model);
rec_test(model);

model = bp_finetune(model);

load('bp_finetuned_model.mat'); %_synthetic_100acc_largenet');
